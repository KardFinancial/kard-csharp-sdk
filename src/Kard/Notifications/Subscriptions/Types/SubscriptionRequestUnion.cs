...
public record SubscriptionRequestUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.Notifications.SubscriptionRequest"/> if <see cref="Type"/> is 'subscription', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'subscription'.</exception>
    public Kard.Notifications.SubscriptionRequest AsSubscription() =>
        IsSubscription
            ? (Kard.Notifications.SubscriptionRequest)Value!
            : throw new global::System.Exception(
                "SubscriptionRequestUnion.Type is not 'subscription'"
            );

    public T Match<T>(
        Func<Kard.Notifications.SubscriptionRequest, T> onSubscription,
    ...
    public void Visit(
        Action<Kard.Notifications.SubscriptionRequest> onSubscription,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.Notifications.SubscriptionRequest"/> and returns true if successful.
    /// </summary>
    public bool TryAsSubscription(out Kard.Notifications.SubscriptionRequest? value)
    {
        if (Type == "subscription")
        {
            value = (Kard.Notifications.SubscriptionRequest)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<SubscriptionRequestUnion>
    {
        ...
        )
        {
            ...
            {
                "subscription" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.Notifications.SubscriptionRequest?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.Notifications.SubscriptionRequest"
                        ),
    ...
    {
        public Subscription(Kard.Notifications.SubscriptionRequest value)
        {
            Value = value;
        }

        internal Kard.Notifications.SubscriptionRequest Value { get; set; }
