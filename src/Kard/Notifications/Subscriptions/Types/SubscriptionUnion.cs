...
public record SubscriptionUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.Notifications.Subscription"/> if <see cref="Type"/> is 'subscription', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'subscription'.</exception>
    public Kard.Notifications.Subscription AsSubscription() =>
        IsSubscription
            ? (Kard.Notifications.Subscription)Value!
            : throw new global::System.Exception("SubscriptionUnion.Type is not 'subscription'");

    public T Match<T>(
        Func<Kard.Notifications.Subscription, T> onSubscription,
    ...
    public void Visit(
        Action<Kard.Notifications.Subscription> onSubscription,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.Notifications.Subscription"/> and returns true if successful.
    /// </summary>
    public bool TryAsSubscription(out Kard.Notifications.Subscription? value)
    {
        if (Type == "subscription")
        {
            value = (Kard.Notifications.Subscription)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<SubscriptionUnion>
    {
        ...
        )
        {
            ...
            {
                "subscription" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.Notifications.Subscription?>(options)
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.Notifications.Subscription"
                        ),
    ...
    {
        public Subscription(Kard.Notifications.Subscription value)
        {
            Value = value;
        }

        internal Kard.Notifications.Subscription Value { get; set; }
