...
public record CreateAuditRequestDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.AuditRequestData"/> if <see cref="Type"/> is 'audit', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'audit'.</exception>
    public Kard.AuditRequestData AsAudit() =>
        IsAudit
            ? (Kard.AuditRequestData)Value!
            : throw new global::System.Exception("CreateAuditRequestDataUnion.Type is not 'audit'");

    public T Match<T>(Func<Kard.AuditRequestData, T> onAudit, Func<string, object?, T> onUnknown_)
    {
        return Type switch
        {
    ...
    public void Visit(Action<Kard.AuditRequestData> onAudit, Action<string, object?> onUnknown_)
    {
        switch (Type)
        {
            case "audit":
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.AuditRequestData"/> and returns true if successful.
    /// </summary>
    public bool TryAsAudit(out Kard.AuditRequestData? value)
    {
        if (Type == "audit")
        {
            value = (Kard.AuditRequestData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<CreateAuditRequestDataUnion>
    {
        ...
        )
        {
            ...
            {
                "audit" => jsonWithoutDiscriminator.Deserialize<Kard.AuditRequestData?>(options)
                    ?? throw new JsonException("Failed to deserialize Kard.AuditRequestData"),
    ...
    public struct Audit
    {
        public Audit(Kard.AuditRequestData value)
        {
            Value = value;
        }

        internal Kard.AuditRequestData Value { get; set; }
