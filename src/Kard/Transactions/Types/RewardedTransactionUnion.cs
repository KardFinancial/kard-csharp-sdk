...
public record RewardedTransactionUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.RewardedTransaction"/> if <see cref="Type"/> is 'rewardedTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'rewardedTransaction'.</exception>
    public Kard.RewardedTransaction AsRewardedTransaction() =>
        IsRewardedTransaction
            ? (Kard.RewardedTransaction)Value!
            : throw new global::System.Exception(
                "RewardedTransactionUnion.Type is not 'rewardedTransaction'"
            );

    /// <summary>
    /// Returns the value as a <see cref="Kard.ApprovedTransaction"/> if <see cref="Type"/> is 'approvedTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'approvedTransaction'.</exception>
    public Kard.ApprovedTransaction AsApprovedTransaction() =>
        IsApprovedTransaction
            ? (Kard.ApprovedTransaction)Value!
            : throw new global::System.Exception(
                "RewardedTransactionUnion.Type is not 'approvedTransaction'"
            );

    public T Match<T>(
        Func<Kard.RewardedTransaction, T> onRewardedTransaction,
        Func<Kard.ApprovedTransaction, T> onApprovedTransaction,
    ...
    public void Visit(
        Action<Kard.RewardedTransaction> onRewardedTransaction,
        Action<Kard.ApprovedTransaction> onApprovedTransaction,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.RewardedTransaction"/> and returns true if successful.
    /// </summary>
    public bool TryAsRewardedTransaction(out Kard.RewardedTransaction? value)
    {
        if (Type == "rewardedTransaction")
        {
            value = (Kard.RewardedTransaction)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.ApprovedTransaction"/> and returns true if successful.
    /// </summary>
    public bool TryAsApprovedTransaction(out Kard.ApprovedTransaction? value)
    {
        if (Type == "approvedTransaction")
        {
            value = (Kard.ApprovedTransaction)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<RewardedTransactionUnion>
    {
        ...
        )
        {
            ...
            {
                "rewardedTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.RewardedTransaction?>(options)
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.RewardedTransaction"
                        ),
                "approvedTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.ApprovedTransaction?>(options)
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.ApprovedTransaction"
                        ),
    ...
    {
        public RewardedTransaction(Kard.RewardedTransaction value)
        {
            Value = value;
        }

        internal Kard.RewardedTransaction Value { get; set; }
    ...
    {
        public ApprovedTransaction(Kard.ApprovedTransaction value)
        {
            Value = value;
        }

        internal Kard.ApprovedTransaction Value { get; set; }
