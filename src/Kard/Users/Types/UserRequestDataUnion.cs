...
public record UserRequestDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.UserRequestData"/> if <see cref="Type"/> is 'user', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'user'.</exception>
    public Kard.UserRequestData AsUser() =>
        IsUser
            ? (Kard.UserRequestData)Value!
            : throw new global::System.Exception("UserRequestDataUnion.Type is not 'user'");

    public T Match<T>(Func<Kard.UserRequestData, T> onUser, Func<string, object?, T> onUnknown_)
    {
        return Type switch
        {
            "user" => onUser(AsUser()),
    ...
    public void Visit(Action<Kard.UserRequestData> onUser, Action<string, object?> onUnknown_)
    {
        switch (Type)
        {
            case "user":
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.UserRequestData"/> and returns true if successful.
    /// </summary>
    public bool TryAsUser(out Kard.UserRequestData? value)
    {
        if (Type == "user")
        {
            value = (Kard.UserRequestData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<UserRequestDataUnion>
    {
        ...
        )
        {
            ...
            {
                "user" => jsonWithoutDiscriminator.Deserialize<Kard.UserRequestData?>(options)
                    ?? throw new JsonException("Failed to deserialize Kard.UserRequestData"),
    ...
    public struct User
    {
        public User(Kard.UserRequestData value)
        {
            Value = value;
        }

        internal Kard.UserRequestData Value { get; set; }

        public override string ToString() => Value.ToString() ?? "null";

        public static implicit operator UserRequestDataUnion.User(Kard.UserRequestData value) =>
            new(value);
    }
}
