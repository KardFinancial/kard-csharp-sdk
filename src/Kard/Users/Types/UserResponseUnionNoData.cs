...
public record UserResponseUnionNoData
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.UserResponseNoData"/> if <see cref="Type"/> is 'user', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'user'.</exception>
    public Kard.UserResponseNoData AsUser() =>
        IsUser
            ? (Kard.UserResponseNoData)Value!
            : throw new global::System.Exception("UserResponseUnionNoData.Type is not 'user'");

    public T Match<T>(Func<Kard.UserResponseNoData, T> onUser, Func<string, object?, T> onUnknown_)
    {
        return Type switch
        {
            "user" => onUser(AsUser()),
    ...
    public void Visit(Action<Kard.UserResponseNoData> onUser, Action<string, object?> onUnknown_)
    {
        switch (Type)
        {
            case "user":
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.UserResponseNoData"/> and returns true if successful.
    /// </summary>
    public bool TryAsUser(out Kard.UserResponseNoData? value)
    {
        if (Type == "user")
        {
            value = (Kard.UserResponseNoData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<UserResponseUnionNoData>
    {
        ...
        )
        {
            ...
            {
                "user" => jsonWithoutDiscriminator.Deserialize<Kard.UserResponseNoData?>(options)
                    ?? throw new JsonException("Failed to deserialize Kard.UserResponseNoData"),
    ...
    public struct User
    {
        public User(Kard.UserResponseNoData value)
        {
            Value = value;
        }

        internal Kard.UserResponseNoData Value { get; set; }
