...
public record CreateUploadPartDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.TransactionsRequest"/> if <see cref="Type"/> is 'historicalTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransaction'.</exception>
    public Kard.TransactionsRequest AsHistoricalTransaction() =>
    ...
    public T Match<T>(
        Func<Kard.TransactionsRequest, T> onHistoricalTransaction,
    ...
    public void Visit(
        Action<Kard.TransactionsRequest> onHistoricalTransaction,
    ...
    /// </summary>
    public bool TryAsHistoricalTransaction(out Kard.TransactionsRequest? value)
    {
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.TransactionsRequest?>(options)
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.TransactionsRequest"
                        ),
    ...
    {
        public HistoricalTransaction(Kard.TransactionsRequest value)
        {
            Value = value;
        }
