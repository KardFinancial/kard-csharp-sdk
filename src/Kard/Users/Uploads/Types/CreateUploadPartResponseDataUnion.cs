...
public record CreateUploadPartResponseDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.Users.CreateUploadPartResponseData"/> if <see cref="Type"/> is 'historicalTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransaction'.</exception>
    public Kard.Users.CreateUploadPartResponseData AsHistoricalTransaction() =>
        IsHistoricalTransaction
            ? (Kard.Users.CreateUploadPartResponseData)Value!
            : throw new global::System.Exception(
                "CreateUploadPartResponseDataUnion.Type is not 'historicalTransaction'"
            );

    public T Match<T>(
        Func<Kard.Users.CreateUploadPartResponseData, T> onHistoricalTransaction,
    ...
    public void Visit(
        Action<Kard.Users.CreateUploadPartResponseData> onHistoricalTransaction,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.Users.CreateUploadPartResponseData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransaction(out Kard.Users.CreateUploadPartResponseData? value)
    {
        if (Type == "historicalTransaction")
        {
            value = (Kard.Users.CreateUploadPartResponseData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.Users.CreateUploadPartResponseData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.Users.CreateUploadPartResponseData"
                        ),
    ...
    {
        public HistoricalTransaction(Kard.Users.CreateUploadPartResponseData value)
        {
            Value = value;
        }

        internal Kard.Users.CreateUploadPartResponseData Value { get; set; }
