...
public record CreateUploadResponseDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.Users.CreateUploadResponseData"/> if <see cref="Type"/> is 'historicalTransactionStart', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransactionStart'.</exception>
    public Kard.Users.CreateUploadResponseData AsHistoricalTransactionStart() =>
        IsHistoricalTransactionStart
            ? (Kard.Users.CreateUploadResponseData)Value!
            : throw new global::System.Exception(
                "CreateUploadResponseDataUnion.Type is not 'historicalTransactionStart'"
            );

    public T Match<T>(
        Func<Kard.Users.CreateUploadResponseData, T> onHistoricalTransactionStart,
    ...
    public void Visit(
        Action<Kard.Users.CreateUploadResponseData> onHistoricalTransactionStart,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.Users.CreateUploadResponseData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransactionStart(out Kard.Users.CreateUploadResponseData? value)
    {
        if (Type == "historicalTransactionStart")
        {
            value = (Kard.Users.CreateUploadResponseData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransactionStart" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.Users.CreateUploadResponseData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.Users.CreateUploadResponseData"
                        ),
    ...
    {
        public HistoricalTransactionStart(Kard.Users.CreateUploadResponseData value)
        {
            Value = value;
        }

        internal Kard.Users.CreateUploadResponseData Value { get; set; }
