...
public record UpdateUploadRequestDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.Users.HistoricalTransactionCompleteNoData"/> if <see cref="Type"/> is 'historicalTransactionComplete', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransactionComplete'.</exception>
    public Kard.Users.HistoricalTransactionCompleteNoData AsHistoricalTransactionComplete() =>
        IsHistoricalTransactionComplete
            ? (Kard.Users.HistoricalTransactionCompleteNoData)Value!
            : throw new global::System.Exception(
                "UpdateUploadRequestDataUnion.Type is not 'historicalTransactionComplete'"
            );

    public T Match<T>(
        Func<Kard.Users.HistoricalTransactionCompleteNoData, T> onHistoricalTransactionComplete,
    ...
    public void Visit(
        Action<Kard.Users.HistoricalTransactionCompleteNoData> onHistoricalTransactionComplete,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.Users.HistoricalTransactionCompleteNoData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransactionComplete(
        out Kard.Users.HistoricalTransactionCompleteNoData? value
    )
    {
        if (Type == "historicalTransactionComplete")
        {
            value = (Kard.Users.HistoricalTransactionCompleteNoData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransactionComplete" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.Users.HistoricalTransactionCompleteNoData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.Users.HistoricalTransactionCompleteNoData"
                        ),
    ...
    {
        public HistoricalTransactionComplete(Kard.Users.HistoricalTransactionCompleteNoData value)
        {
            Value = value;
        }

        internal Kard.Users.HistoricalTransactionCompleteNoData Value { get; set; }
