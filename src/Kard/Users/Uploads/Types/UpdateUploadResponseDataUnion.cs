...
public record UpdateUploadResponseDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="Kard.Users.UpdateUploadResponseData"/> if <see cref="Type"/> is 'historicalTransactionComplete', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransactionComplete'.</exception>
    public Kard.Users.UpdateUploadResponseData AsHistoricalTransactionComplete() =>
        IsHistoricalTransactionComplete
            ? (Kard.Users.UpdateUploadResponseData)Value!
            : throw new global::System.Exception(
                "UpdateUploadResponseDataUnion.Type is not 'historicalTransactionComplete'"
            );

    public T Match<T>(
        Func<Kard.Users.UpdateUploadResponseData, T> onHistoricalTransactionComplete,
    ...
    public void Visit(
        Action<Kard.Users.UpdateUploadResponseData> onHistoricalTransactionComplete,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="Kard.Users.UpdateUploadResponseData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransactionComplete(out Kard.Users.UpdateUploadResponseData? value)
    {
        if (Type == "historicalTransactionComplete")
        {
            value = (Kard.Users.UpdateUploadResponseData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransactionComplete" =>
                    jsonWithoutDiscriminator.Deserialize<Kard.Users.UpdateUploadResponseData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize Kard.Users.UpdateUploadResponseData"
                        ),
    ...
    {
        public HistoricalTransactionComplete(Kard.Users.UpdateUploadResponseData value)
        {
            Value = value;
        }

        internal Kard.Users.UpdateUploadResponseData Value { get; set; }
