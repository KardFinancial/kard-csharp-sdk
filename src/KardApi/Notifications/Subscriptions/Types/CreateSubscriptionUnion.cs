...
public record CreateSubscriptionUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.Notifications.CreatedSubscription"/> if <see cref="Type"/> is 'subscription', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'subscription'.</exception>
    public KardApi.Notifications.CreatedSubscription AsSubscription() =>
        IsSubscription
            ? (KardApi.Notifications.CreatedSubscription)Value!
            : throw new global::System.Exception(
                "CreateSubscriptionUnion.Type is not 'subscription'"
            );

    public T Match<T>(
        Func<KardApi.Notifications.CreatedSubscription, T> onSubscription,
    ...
    public void Visit(
        Action<KardApi.Notifications.CreatedSubscription> onSubscription,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Notifications.CreatedSubscription"/> and returns true if successful.
    /// </summary>
    public bool TryAsSubscription(out KardApi.Notifications.CreatedSubscription? value)
    {
        if (Type == "subscription")
        {
            value = (KardApi.Notifications.CreatedSubscription)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<CreateSubscriptionUnion>
    {
        ...
        )
        {
            ...
            {
                "subscription" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Notifications.CreatedSubscription?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Notifications.CreatedSubscription"
                        ),
    ...
    {
        public Subscription(KardApi.Notifications.CreatedSubscription value)
        {
            Value = value;
        }

        internal KardApi.Notifications.CreatedSubscription Value { get; set; }
