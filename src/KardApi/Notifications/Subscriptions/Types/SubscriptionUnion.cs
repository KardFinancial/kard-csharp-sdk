...
public record SubscriptionUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.Notifications.Subscription"/> if <see cref="Type"/> is 'subscription', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'subscription'.</exception>
    public KardApi.Notifications.Subscription AsSubscription() =>
        IsSubscription
            ? (KardApi.Notifications.Subscription)Value!
            : throw new global::System.Exception("SubscriptionUnion.Type is not 'subscription'");

    public T Match<T>(
        Func<KardApi.Notifications.Subscription, T> onSubscription,
    ...
    public void Visit(
        Action<KardApi.Notifications.Subscription> onSubscription,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Notifications.Subscription"/> and returns true if successful.
    /// </summary>
    public bool TryAsSubscription(out KardApi.Notifications.Subscription? value)
    {
        if (Type == "subscription")
        {
            value = (KardApi.Notifications.Subscription)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<SubscriptionUnion>
    {
        ...
        )
        {
            ...
            {
                "subscription" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Notifications.Subscription?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Notifications.Subscription"
                        ),
    ...
    {
        public Subscription(KardApi.Notifications.Subscription value)
        {
            Value = value;
        }

        internal KardApi.Notifications.Subscription Value { get; set; }
