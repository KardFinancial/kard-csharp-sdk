...
public record CreatePlacementDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.Organizations.CreateMainPagePlacementData"/> if <see cref="Type"/> is 'placementMainPage', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'placementMainPage'.</exception>
    public KardApi.Organizations.CreateMainPagePlacementData AsPlacementMainPage() =>
        IsPlacementMainPage
            ? (KardApi.Organizations.CreateMainPagePlacementData)Value!
            : throw new global::System.Exception(
                "CreatePlacementDataUnion.Type is not 'placementMainPage'"
            );

    /// <summary>
    /// Returns the value as a <see cref="KardApi.Organizations.CreatePushNotificationPlacementData"/> if <see cref="Type"/> is 'placementPushNotification', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'placementPushNotification'.</exception>
    public KardApi.Organizations.CreatePushNotificationPlacementData AsPlacementPushNotification() =>
        IsPlacementPushNotification
            ? (KardApi.Organizations.CreatePushNotificationPlacementData)Value!
            : throw new global::System.Exception(
                "CreatePlacementDataUnion.Type is not 'placementPushNotification'"
            );

    public T Match<T>(
        Func<KardApi.Organizations.CreateMainPagePlacementData, T> onPlacementMainPage,
        Func<
            KardApi.Organizations.CreatePushNotificationPlacementData,
            T
    ...
    public void Visit(
        Action<KardApi.Organizations.CreateMainPagePlacementData> onPlacementMainPage,
        Action<KardApi.Organizations.CreatePushNotificationPlacementData> onPlacementPushNotification,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Organizations.CreateMainPagePlacementData"/> and returns true if successful.
    /// </summary>
    public bool TryAsPlacementMainPage(out KardApi.Organizations.CreateMainPagePlacementData? value)
    {
        if (Type == "placementMainPage")
        {
            value = (KardApi.Organizations.CreateMainPagePlacementData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Organizations.CreatePushNotificationPlacementData"/> and returns true if successful.
    /// </summary>
    public bool TryAsPlacementPushNotification(
        out KardApi.Organizations.CreatePushNotificationPlacementData? value
    )
    {
        if (Type == "placementPushNotification")
        {
            value = (KardApi.Organizations.CreatePushNotificationPlacementData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<CreatePlacementDataUnion>
    {
        ...
        )
        {
            ...
            {
                "placementMainPage" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Organizations.CreateMainPagePlacementData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Organizations.CreateMainPagePlacementData"
                        ),
                "placementPushNotification" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Organizations.CreatePushNotificationPlacementData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Organizations.CreatePushNotificationPlacementData"
                        ),
    ...
    {
        public PlacementMainPage(KardApi.Organizations.CreateMainPagePlacementData value)
        {
            Value = value;
        }

        internal KardApi.Organizations.CreateMainPagePlacementData Value { get; set; }
    ...
    public struct PlacementPushNotification
    {
        ...
        internal KardApi.Organizations.CreatePushNotificationPlacementData Value { get; set; }
