...
public record PlacementFormatUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.Organizations.MainPagePlacementData"/> if <see cref="Type"/> is 'placementMainPage', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'placementMainPage'.</exception>
    public KardApi.Organizations.MainPagePlacementData AsPlacementMainPage() =>
        IsPlacementMainPage
            ? (KardApi.Organizations.MainPagePlacementData)Value!
            : throw new global::System.Exception(
                "PlacementFormatUnion.Type is not 'placementMainPage'"
            );

    /// <summary>
    /// Returns the value as a <see cref="KardApi.Organizations.PushNotificationPlacementData"/> if <see cref="Type"/> is 'placementPushNotification', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'placementPushNotification'.</exception>
    public KardApi.Organizations.PushNotificationPlacementData AsPlacementPushNotification() =>
        IsPlacementPushNotification
            ? (KardApi.Organizations.PushNotificationPlacementData)Value!
            : throw new global::System.Exception(
                "PlacementFormatUnion.Type is not 'placementPushNotification'"
            );

    public T Match<T>(
        Func<KardApi.Organizations.MainPagePlacementData, T> onPlacementMainPage,
        Func<KardApi.Organizations.PushNotificationPlacementData, T> onPlacementPushNotification,
    ...
    public void Visit(
        Action<KardApi.Organizations.MainPagePlacementData> onPlacementMainPage,
        Action<KardApi.Organizations.PushNotificationPlacementData> onPlacementPushNotification,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Organizations.MainPagePlacementData"/> and returns true if successful.
    /// </summary>
    public bool TryAsPlacementMainPage(out KardApi.Organizations.MainPagePlacementData? value)
    {
        if (Type == "placementMainPage")
        {
            value = (KardApi.Organizations.MainPagePlacementData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Organizations.PushNotificationPlacementData"/> and returns true if successful.
    /// </summary>
    public bool TryAsPlacementPushNotification(
        out KardApi.Organizations.PushNotificationPlacementData? value
    )
    {
        if (Type == "placementPushNotification")
        {
            value = (KardApi.Organizations.PushNotificationPlacementData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<PlacementFormatUnion>
    {
        ...
        )
        {
            ...
            {
                "placementMainPage" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Organizations.MainPagePlacementData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Organizations.MainPagePlacementData"
                        ),
                "placementPushNotification" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Organizations.PushNotificationPlacementData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Organizations.PushNotificationPlacementData"
                        ),
    ...
    {
        public PlacementMainPage(KardApi.Organizations.MainPagePlacementData value)
        {
            Value = value;
        }

        internal KardApi.Organizations.MainPagePlacementData Value { get; set; }
    ...
    {
        public PlacementPushNotification(KardApi.Organizations.PushNotificationPlacementData value)
        {
            Value = value;
        }

        internal KardApi.Organizations.PushNotificationPlacementData Value { get; set; }
