...
public record CreateAuditResponseDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.AuditResponseData"/> if <see cref="Type"/> is 'audit', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'audit'.</exception>
    public KardApi.AuditResponseData AsAudit() =>
        IsAudit
            ? (KardApi.AuditResponseData)Value!
            : throw new global::System.Exception(
                "CreateAuditResponseDataUnion.Type is not 'audit'"
            );

    public T Match<T>(
        Func<KardApi.AuditResponseData, T> onAudit,
    ...
    public void Visit(Action<KardApi.AuditResponseData> onAudit, Action<string, object?> onUnknown_)
    {
        switch (Type)
        {
            case "audit":
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.AuditResponseData"/> and returns true if successful.
    /// </summary>
    public bool TryAsAudit(out KardApi.AuditResponseData? value)
    {
        if (Type == "audit")
        {
            value = (KardApi.AuditResponseData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<CreateAuditResponseDataUnion>
    {
        ...
        )
        {
            ...
            {
                "audit" => jsonWithoutDiscriminator.Deserialize<KardApi.AuditResponseData?>(options)
                    ?? throw new JsonException("Failed to deserialize KardApi.AuditResponseData"),
    ...
    public struct Audit
    {
        public Audit(KardApi.AuditResponseData value)
        {
            Value = value;
        }

        internal KardApi.AuditResponseData Value { get; set; }
