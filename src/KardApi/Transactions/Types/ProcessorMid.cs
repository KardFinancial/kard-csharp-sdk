...
public record ProcessorMid
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.VisaMid"/> if <see cref="Processor"/> is 'VISA', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Processor"/> is not 'VISA'.</exception>
    public KardApi.VisaMid AsVisa() =>
        IsVisa
            ? (KardApi.VisaMid)Value!
            : throw new global::System.Exception("ProcessorMid.Processor is not 'VISA'");

    public T Match<T>(Func<KardApi.VisaMid, T> onVisa, Func<string, object?, T> onUnknown_)
    {
    ...
    public void Visit(Action<KardApi.VisaMid> onVisa, Action<string, object?> onUnknown_)
    {
        switch (Processor)
        {
            case "VISA":
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.VisaMid"/> and returns true if successful.
    /// </summary>
    public bool TryAsVisa(out KardApi.VisaMid? value)
    {
        if (Processor == "VISA")
        {
            value = (KardApi.VisaMid)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<ProcessorMid>
    {
        ...
        )
        {
            ...
            {
                "VISA" => jsonWithoutDiscriminator.Deserialize<KardApi.VisaMid?>(options)
                    ?? throw new JsonException("Failed to deserialize KardApi.VisaMid"),
    ...
    public struct Visa
    {
        public Visa(KardApi.VisaMid value)
        {
            Value = value;
        }

        internal KardApi.VisaMid Value { get; set; }

        public override string ToString() => Value.ToString() ?? "null";

        public static implicit operator ProcessorMid.Visa(KardApi.VisaMid value) => new(value);
    }
}
