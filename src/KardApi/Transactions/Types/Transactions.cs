...
public record Transactions
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.TransactionsRequest"/> if <see cref="Type"/> is 'transaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'transaction'.</exception>
    public KardApi.TransactionsRequest AsTransaction() =>
        IsTransaction
            ? (KardApi.TransactionsRequest)Value!
            : throw new global::System.Exception("Transactions.Type is not 'transaction'");

    /// <summary>
    /// Returns the value as a <see cref="KardApi.MatchedTransactionsRequest"/> if <see cref="Type"/> is 'matchedTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'matchedTransaction'.</exception>
    public KardApi.MatchedTransactionsRequest AsMatchedTransaction() =>
        IsMatchedTransaction
            ? (KardApi.MatchedTransactionsRequest)Value!
            : throw new global::System.Exception("Transactions.Type is not 'matchedTransaction'");

    /// <summary>
    /// Returns the value as a <see cref="KardApi.CoreTransactionRequest"/> if <see cref="Type"/> is 'coreTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'coreTransaction'.</exception>
    public KardApi.CoreTransactionRequest AsCoreTransaction() =>
        IsCoreTransaction
            ? (KardApi.CoreTransactionRequest)Value!
            : throw new global::System.Exception("Transactions.Type is not 'coreTransaction'");

    public T Match<T>(
        Func<KardApi.TransactionsRequest, T> onTransaction,
        Func<KardApi.MatchedTransactionsRequest, T> onMatchedTransaction,
        Func<KardApi.CoreTransactionRequest, T> onCoreTransaction,
    ...
    public void Visit(
        Action<KardApi.TransactionsRequest> onTransaction,
        Action<KardApi.MatchedTransactionsRequest> onMatchedTransaction,
        Action<KardApi.CoreTransactionRequest> onCoreTransaction,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.TransactionsRequest"/> and returns true if successful.
    /// </summary>
    public bool TryAsTransaction(out KardApi.TransactionsRequest? value)
    {
        if (Type == "transaction")
        {
            value = (KardApi.TransactionsRequest)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.MatchedTransactionsRequest"/> and returns true if successful.
    /// </summary>
    public bool TryAsMatchedTransaction(out KardApi.MatchedTransactionsRequest? value)
    {
        if (Type == "matchedTransaction")
        {
            value = (KardApi.MatchedTransactionsRequest)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.CoreTransactionRequest"/> and returns true if successful.
    /// </summary>
    public bool TryAsCoreTransaction(out KardApi.CoreTransactionRequest? value)
    {
        if (Type == "coreTransaction")
        {
            value = (KardApi.CoreTransactionRequest)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<Transactions>
    {
        ...
        )
        {
            ...
            {
                "transaction" => jsonWithoutDiscriminator.Deserialize<KardApi.TransactionsRequest?>(
                    options
                ) ?? throw new JsonException("Failed to deserialize KardApi.TransactionsRequest"),
                "matchedTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.MatchedTransactionsRequest?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.MatchedTransactionsRequest"
                        ),
                "coreTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.CoreTransactionRequest?>(options)
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.CoreTransactionRequest"
                        ),
    ...
    {
        public Transaction(KardApi.TransactionsRequest value)
        {
            Value = value;
        }

        internal KardApi.TransactionsRequest Value { get; set; }
    ...
    {
        public MatchedTransaction(KardApi.MatchedTransactionsRequest value)
        {
            Value = value;
        }

        internal KardApi.MatchedTransactionsRequest Value { get; set; }
    ...
    {
        public CoreTransaction(KardApi.CoreTransactionRequest value)
        {
            Value = value;
        }

        internal KardApi.CoreTransactionRequest Value { get; set; }
