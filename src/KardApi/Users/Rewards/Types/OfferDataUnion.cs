...
public record OfferDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.Users.StandardOffer"/> if <see cref="Type"/> is 'standardOffer', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'standardOffer'.</exception>
    public KardApi.Users.StandardOffer AsStandardOffer() =>
        IsStandardOffer
            ? (KardApi.Users.StandardOffer)Value!
            : throw new global::System.Exception("OfferDataUnion.Type is not 'standardOffer'");

    public T Match<T>(
        Func<KardApi.Users.StandardOffer, T> onStandardOffer,
    ...
    public void Visit(
        Action<KardApi.Users.StandardOffer> onStandardOffer,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Users.StandardOffer"/> and returns true if successful.
    /// </summary>
    public bool TryAsStandardOffer(out KardApi.Users.StandardOffer? value)
    {
        if (Type == "standardOffer")
        {
            value = (KardApi.Users.StandardOffer)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<OfferDataUnion>
    {
        ...
        )
        {
            ...
            {
                "standardOffer" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Users.StandardOffer?>(options)
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Users.StandardOffer"
                        ),
    ...
    {
        public StandardOffer(KardApi.Users.StandardOffer value)
        {
            Value = value;
        }

        internal KardApi.Users.StandardOffer Value { get; set; }
