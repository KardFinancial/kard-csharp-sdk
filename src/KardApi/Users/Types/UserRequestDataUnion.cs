...
public record UserRequestDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.UserRequestData"/> if <see cref="Type"/> is 'user', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'user'.</exception>
    public KardApi.UserRequestData AsUser() =>
        IsUser
            ? (KardApi.UserRequestData)Value!
            : throw new global::System.Exception("UserRequestDataUnion.Type is not 'user'");

    public T Match<T>(Func<KardApi.UserRequestData, T> onUser, Func<string, object?, T> onUnknown_)
    {
        return Type switch
        {
            "user" => onUser(AsUser()),
    ...
    public void Visit(Action<KardApi.UserRequestData> onUser, Action<string, object?> onUnknown_)
    {
        switch (Type)
        {
            case "user":
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.UserRequestData"/> and returns true if successful.
    /// </summary>
    public bool TryAsUser(out KardApi.UserRequestData? value)
    {
        if (Type == "user")
        {
            value = (KardApi.UserRequestData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<UserRequestDataUnion>
    {
        ...
        )
        {
            ...
            {
                "user" => jsonWithoutDiscriminator.Deserialize<KardApi.UserRequestData?>(options)
                    ?? throw new JsonException("Failed to deserialize KardApi.UserRequestData"),
    ...
    public struct User
    {
        public User(KardApi.UserRequestData value)
        {
            Value = value;
        }

        internal KardApi.UserRequestData Value { get; set; }

        public override string ToString() => Value.ToString() ?? "null";

        public static implicit operator UserRequestDataUnion.User(KardApi.UserRequestData value) =>
            new(value);
    }
}
