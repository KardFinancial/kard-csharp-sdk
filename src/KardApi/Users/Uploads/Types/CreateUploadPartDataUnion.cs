...
public record CreateUploadPartDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.TransactionsRequest"/> if <see cref="Type"/> is 'historicalTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransaction'.</exception>
    public KardApi.TransactionsRequest AsHistoricalTransaction() =>
    ...
    public T Match<T>(
        Func<KardApi.TransactionsRequest, T> onHistoricalTransaction,
    ...
    public void Visit(
        Action<KardApi.TransactionsRequest> onHistoricalTransaction,
    ...
    /// </summary>
    public bool TryAsHistoricalTransaction(out KardApi.TransactionsRequest? value)
    {
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.TransactionsRequest?>(options)
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.TransactionsRequest"
                        ),
    ...
    {
        public HistoricalTransaction(KardApi.TransactionsRequest value)
        {
            Value = value;
        }
