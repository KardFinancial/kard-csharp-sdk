...
public record CreateUploadRequestDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.Users.StartHistoricalUploadNoData"/> if <see cref="Type"/> is 'historicalTransactionStart', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransactionStart'.</exception>
    public KardApi.Users.StartHistoricalUploadNoData AsHistoricalTransactionStart() =>
        IsHistoricalTransactionStart
            ? (KardApi.Users.StartHistoricalUploadNoData)Value!
            : throw new global::System.Exception(
                "CreateUploadRequestDataUnion.Type is not 'historicalTransactionStart'"
            );

    public T Match<T>(
        Func<KardApi.Users.StartHistoricalUploadNoData, T> onHistoricalTransactionStart,
    ...
    public void Visit(
        Action<KardApi.Users.StartHistoricalUploadNoData> onHistoricalTransactionStart,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Users.StartHistoricalUploadNoData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransactionStart(
        out KardApi.Users.StartHistoricalUploadNoData? value
    )
    {
        if (Type == "historicalTransactionStart")
        {
            value = (KardApi.Users.StartHistoricalUploadNoData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransactionStart" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Users.StartHistoricalUploadNoData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Users.StartHistoricalUploadNoData"
                        ),
    ...
    {
        public HistoricalTransactionStart(KardApi.Users.StartHistoricalUploadNoData value)
        {
            Value = value;
        }

        internal KardApi.Users.StartHistoricalUploadNoData Value { get; set; }
