...
public record UpdateUploadResponseDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardApi.Users.UpdateUploadResponseData"/> if <see cref="Type"/> is 'historicalTransactionComplete', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransactionComplete'.</exception>
    public KardApi.Users.UpdateUploadResponseData AsHistoricalTransactionComplete() =>
        IsHistoricalTransactionComplete
            ? (KardApi.Users.UpdateUploadResponseData)Value!
            : throw new global::System.Exception(
                "UpdateUploadResponseDataUnion.Type is not 'historicalTransactionComplete'"
            );

    public T Match<T>(
        Func<KardApi.Users.UpdateUploadResponseData, T> onHistoricalTransactionComplete,
    ...
    public void Visit(
        Action<KardApi.Users.UpdateUploadResponseData> onHistoricalTransactionComplete,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardApi.Users.UpdateUploadResponseData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransactionComplete(
        out KardApi.Users.UpdateUploadResponseData? value
    )
    {
        if (Type == "historicalTransactionComplete")
        {
            value = (KardApi.Users.UpdateUploadResponseData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransactionComplete" =>
                    jsonWithoutDiscriminator.Deserialize<KardApi.Users.UpdateUploadResponseData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardApi.Users.UpdateUploadResponseData"
                        ),
    ...
    {
        public HistoricalTransactionComplete(KardApi.Users.UpdateUploadResponseData value)
        {
            Value = value;
        }

        internal KardApi.Users.UpdateUploadResponseData Value { get; set; }
