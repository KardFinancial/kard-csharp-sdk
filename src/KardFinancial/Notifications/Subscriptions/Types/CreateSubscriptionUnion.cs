...
public record CreateSubscriptionUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.Notifications.CreatedSubscription"/> if <see cref="Type"/> is 'subscription', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'subscription'.</exception>
    public KardFinancial.Notifications.CreatedSubscription AsSubscription() =>
        IsSubscription
            ? (KardFinancial.Notifications.CreatedSubscription)Value!
            : throw new global::System.Exception(
                "CreateSubscriptionUnion.Type is not 'subscription'"
            );

    public T Match<T>(
        Func<KardFinancial.Notifications.CreatedSubscription, T> onSubscription,
    ...
    public void Visit(
        Action<KardFinancial.Notifications.CreatedSubscription> onSubscription,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.Notifications.CreatedSubscription"/> and returns true if successful.
    /// </summary>
    public bool TryAsSubscription(out KardFinancial.Notifications.CreatedSubscription? value)
    {
        if (Type == "subscription")
        {
            value = (KardFinancial.Notifications.CreatedSubscription)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<CreateSubscriptionUnion>
    {
        ...
        )
        {
            ...
            {
                "subscription" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.Notifications.CreatedSubscription?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.Notifications.CreatedSubscription"
                        ),
    ...
    {
        public Subscription(KardFinancial.Notifications.CreatedSubscription value)
        {
            Value = value;
        }

        internal KardFinancial.Notifications.CreatedSubscription Value { get; set; }
