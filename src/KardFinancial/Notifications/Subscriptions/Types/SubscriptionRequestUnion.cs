...
public record SubscriptionRequestUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.Notifications.SubscriptionRequest"/> if <see cref="Type"/> is 'subscription', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'subscription'.</exception>
    public KardFinancial.Notifications.SubscriptionRequest AsSubscription() =>
        IsSubscription
            ? (KardFinancial.Notifications.SubscriptionRequest)Value!
            : throw new global::System.Exception(
                "SubscriptionRequestUnion.Type is not 'subscription'"
            );

    public T Match<T>(
        Func<KardFinancial.Notifications.SubscriptionRequest, T> onSubscription,
    ...
    public void Visit(
        Action<KardFinancial.Notifications.SubscriptionRequest> onSubscription,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.Notifications.SubscriptionRequest"/> and returns true if successful.
    /// </summary>
    public bool TryAsSubscription(out KardFinancial.Notifications.SubscriptionRequest? value)
    {
        if (Type == "subscription")
        {
            value = (KardFinancial.Notifications.SubscriptionRequest)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<SubscriptionRequestUnion>
    {
        ...
        )
        {
            ...
            {
                "subscription" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.Notifications.SubscriptionRequest?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.Notifications.SubscriptionRequest"
                        ),
    ...
    {
        public Subscription(KardFinancial.Notifications.SubscriptionRequest value)
        {
            Value = value;
        }

        internal KardFinancial.Notifications.SubscriptionRequest Value { get; set; }
