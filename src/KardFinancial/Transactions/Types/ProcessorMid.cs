...
public record ProcessorMid
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.VisaMid"/> if <see cref="Processor"/> is 'VISA', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Processor"/> is not 'VISA'.</exception>
    public KardFinancial.VisaMid AsVisa() =>
        IsVisa
            ? (KardFinancial.VisaMid)Value!
            : throw new global::System.Exception("ProcessorMid.Processor is not 'VISA'");

    public T Match<T>(Func<KardFinancial.VisaMid, T> onVisa, Func<string, object?, T> onUnknown_)
    {
    ...
    public void Visit(Action<KardFinancial.VisaMid> onVisa, Action<string, object?> onUnknown_)
    {
        switch (Processor)
        {
            case "VISA":
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.VisaMid"/> and returns true if successful.
    /// </summary>
    public bool TryAsVisa(out KardFinancial.VisaMid? value)
    {
        if (Processor == "VISA")
        {
            value = (KardFinancial.VisaMid)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<ProcessorMid>
    {
        ...
        )
        {
            ...
            {
                "VISA" => jsonWithoutDiscriminator.Deserialize<KardFinancial.VisaMid?>(options)
                    ?? throw new JsonException("Failed to deserialize KardFinancial.VisaMid"),
    ...
    public struct Visa
    {
        public Visa(KardFinancial.VisaMid value)
        {
            Value = value;
        }

        internal KardFinancial.VisaMid Value { get; set; }

        public override string ToString() => Value.ToString() ?? "null";

        public static implicit operator ProcessorMid.Visa(KardFinancial.VisaMid value) =>
            new(value);
    }
}
