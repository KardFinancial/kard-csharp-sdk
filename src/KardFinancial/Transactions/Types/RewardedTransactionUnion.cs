...
public record RewardedTransactionUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.RewardedTransaction"/> if <see cref="Type"/> is 'rewardedTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'rewardedTransaction'.</exception>
    public KardFinancial.RewardedTransaction AsRewardedTransaction() =>
        IsRewardedTransaction
            ? (KardFinancial.RewardedTransaction)Value!
            : throw new global::System.Exception(
                "RewardedTransactionUnion.Type is not 'rewardedTransaction'"
            );

    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.ApprovedTransaction"/> if <see cref="Type"/> is 'approvedTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'approvedTransaction'.</exception>
    public KardFinancial.ApprovedTransaction AsApprovedTransaction() =>
        IsApprovedTransaction
            ? (KardFinancial.ApprovedTransaction)Value!
            : throw new global::System.Exception(
                "RewardedTransactionUnion.Type is not 'approvedTransaction'"
            );

    public T Match<T>(
        Func<KardFinancial.RewardedTransaction, T> onRewardedTransaction,
        Func<KardFinancial.ApprovedTransaction, T> onApprovedTransaction,
    ...
    public void Visit(
        Action<KardFinancial.RewardedTransaction> onRewardedTransaction,
        Action<KardFinancial.ApprovedTransaction> onApprovedTransaction,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.RewardedTransaction"/> and returns true if successful.
    /// </summary>
    public bool TryAsRewardedTransaction(out KardFinancial.RewardedTransaction? value)
    {
        if (Type == "rewardedTransaction")
        {
            value = (KardFinancial.RewardedTransaction)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.ApprovedTransaction"/> and returns true if successful.
    /// </summary>
    public bool TryAsApprovedTransaction(out KardFinancial.ApprovedTransaction? value)
    {
        if (Type == "approvedTransaction")
        {
            value = (KardFinancial.ApprovedTransaction)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<RewardedTransactionUnion>
    {
        ...
        )
        {
            ...
            {
                "rewardedTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.RewardedTransaction?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.RewardedTransaction"
                        ),
                "approvedTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.ApprovedTransaction?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.ApprovedTransaction"
                        ),
    ...
    {
        public RewardedTransaction(KardFinancial.RewardedTransaction value)
        {
            Value = value;
        }

        internal KardFinancial.RewardedTransaction Value { get; set; }
    ...
    {
        public ApprovedTransaction(KardFinancial.ApprovedTransaction value)
        {
            Value = value;
        }

        internal KardFinancial.ApprovedTransaction Value { get; set; }
