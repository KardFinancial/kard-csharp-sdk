...
public record TransactionIncludedResource
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.TransactionMerchantResource"/> if <see cref="Type"/> is 'merchant', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'merchant'.</exception>
    public KardFinancial.TransactionMerchantResource AsMerchant() =>
        IsMerchant
            ? (KardFinancial.TransactionMerchantResource)Value!
            : throw new global::System.Exception(
                "TransactionIncludedResource.Type is not 'merchant'"
            );

    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.TransactionOfferResource"/> if <see cref="Type"/> is 'offer', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'offer'.</exception>
    public KardFinancial.TransactionOfferResource AsOffer() =>
        IsOffer
            ? (KardFinancial.TransactionOfferResource)Value!
            : throw new global::System.Exception("TransactionIncludedResource.Type is not 'offer'");

    public T Match<T>(
        Func<KardFinancial.TransactionMerchantResource, T> onMerchant,
        Func<KardFinancial.TransactionOfferResource, T> onOffer,
    ...
    public void Visit(
        Action<KardFinancial.TransactionMerchantResource> onMerchant,
        Action<KardFinancial.TransactionOfferResource> onOffer,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.TransactionMerchantResource"/> and returns true if successful.
    /// </summary>
    public bool TryAsMerchant(out KardFinancial.TransactionMerchantResource? value)
    {
        if (Type == "merchant")
        {
            value = (KardFinancial.TransactionMerchantResource)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.TransactionOfferResource"/> and returns true if successful.
    /// </summary>
    public bool TryAsOffer(out KardFinancial.TransactionOfferResource? value)
    {
        if (Type == "offer")
        {
            value = (KardFinancial.TransactionOfferResource)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<TransactionIncludedResource>
    {
        ...
        )
        {
            ...
            {
                "merchant" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.TransactionMerchantResource?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.TransactionMerchantResource"
                        ),
                "offer" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.TransactionOfferResource?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.TransactionOfferResource"
                        ),
    ...
    {
        public Merchant(KardFinancial.TransactionMerchantResource value)
        {
            Value = value;
        }

        internal KardFinancial.TransactionMerchantResource Value { get; set; }
    ...
    public struct Offer
    {
        public Offer(KardFinancial.TransactionOfferResource value)
        {
            Value = value;
        }

        internal KardFinancial.TransactionOfferResource Value { get; set; }
