...
public record Transactions
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.TransactionsRequest"/> if <see cref="Type"/> is 'transaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'transaction'.</exception>
    public KardFinancial.TransactionsRequest AsTransaction() =>
        IsTransaction
    ...
    public KardFinancial.CoreTransactionRequest AsCoreTransaction() =>
        IsCoreTransaction
            ? (KardFinancial.CoreTransactionRequest)Value!
    ...
    /// </summary>
    public bool TryAsTransaction(out KardFinancial.TransactionsRequest? value)
    {
        if (Type == "transaction")
        {
    ...
    public bool TryAsCoreTransaction(out KardFinancial.CoreTransactionRequest? value)
    {
        ...
        {
            value = (KardFinancial.CoreTransactionRequest)Value!;
            return true;
        }
        value = null;
        return false;
            ...
            var value = discriminator switch
            {
                ...
                "coreTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.CoreTransactionRequest?>(
                        ...
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.CoreTransactionRequest"
                        ),
    ...
    {
        public Transaction(KardFinancial.TransactionsRequest value)
        {
            Value = value;
        }
    ...
    {
        public CoreTransaction(KardFinancial.CoreTransactionRequest value)
        {
            Value = value;
        }

        internal KardFinancial.CoreTransactionRequest Value { get; set; }
