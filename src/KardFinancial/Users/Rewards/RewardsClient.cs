...
public partial class RewardsClient : IRewardsClient
{
    ...
    {
        var _queryString = new KardFinancial.Core.QueryStringBuilder.Builder(capacity: 10)
    ...
    private async Task<
        WithRawResponse<OneOf<OffersResponseObject, BatchesResponseObject>>
        ...
        if (response.StatusCode is >= 200 and < 400)
        {
            ...
            try
            {
                var responseData = JsonUtils.Deserialize<
                    OneOf<OffersResponseObject, BatchesResponseObject>
                >(responseBody)!;
                return new WithRawResponse<OneOf<OffersResponseObject, BatchesResponseObject>>()
                {
                    Data = responseData,
                    RawResponse = new RawResponse()
                    {
    ...
    {
        var _queryString = new KardFinancial.Core.QueryStringBuilder.Builder(capacity: 14)
            .Add("page[size]", request.PageSize)
            .Add("page[after]", request.PageAfter)
            .Add("page[before]", request.PageBefore)
    ...
    {
        return new WithRawResponseTask<OneOf<OffersResponseObject, BatchesResponseObject>>(
            PlacementContentAsyncCore(
                organizationId,
                userId,
                placementId,
