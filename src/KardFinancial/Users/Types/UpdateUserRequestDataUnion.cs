...
public record UpdateUserRequestDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.UpdateUserRequestData"/> if <see cref="Type"/> is 'user', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'user'.</exception>
    public KardFinancial.UpdateUserRequestData AsUser() =>
        IsUser
            ? (KardFinancial.UpdateUserRequestData)Value!
            : throw new global::System.Exception("UpdateUserRequestDataUnion.Type is not 'user'");

    public T Match<T>(
        Func<KardFinancial.UpdateUserRequestData, T> onUser,
    ...
    public void Visit(
        Action<KardFinancial.UpdateUserRequestData> onUser,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.UpdateUserRequestData"/> and returns true if successful.
    /// </summary>
    public bool TryAsUser(out KardFinancial.UpdateUserRequestData? value)
    {
        if (Type == "user")
        {
            value = (KardFinancial.UpdateUserRequestData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<UpdateUserRequestDataUnion>
    {
        ...
        )
        {
            ...
            {
                "user" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.UpdateUserRequestData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.UpdateUserRequestData"
                        ),
    ...
    public struct User
    {
        public User(KardFinancial.UpdateUserRequestData value)
        {
            Value = value;
        }

        internal KardFinancial.UpdateUserRequestData Value { get; set; }
