...
public record UserResponseUnionNoData
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.UserResponseNoData"/> if <see cref="Type"/> is 'user', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'user'.</exception>
    public KardFinancial.UserResponseNoData AsUser() =>
        IsUser
            ? (KardFinancial.UserResponseNoData)Value!
            : throw new global::System.Exception("UserResponseUnionNoData.Type is not 'user'");

    public T Match<T>(
        Func<KardFinancial.UserResponseNoData, T> onUser,
    ...
    public void Visit(
        Action<KardFinancial.UserResponseNoData> onUser,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.UserResponseNoData"/> and returns true if successful.
    /// </summary>
    public bool TryAsUser(out KardFinancial.UserResponseNoData? value)
    {
        if (Type == "user")
        {
            value = (KardFinancial.UserResponseNoData)Value!;
            return true;
        }
        value = null;
        return false;
    ...
    internal sealed class JsonConverter : JsonConverter<UserResponseUnionNoData>
    {
        ...
        )
        {
            ...
            {
                "user" => jsonWithoutDiscriminator.Deserialize<KardFinancial.UserResponseNoData?>(
                    options
                )
                    ?? throw new JsonException(
                        "Failed to deserialize KardFinancial.UserResponseNoData"
                    ),
    ...
    public struct User
    {
        public User(KardFinancial.UserResponseNoData value)
        {
            Value = value;
        }

        internal KardFinancial.UserResponseNoData Value { get; set; }
