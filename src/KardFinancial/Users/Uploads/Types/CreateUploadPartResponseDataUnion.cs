...
public record CreateUploadPartResponseDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.Users.CreateUploadPartResponseData"/> if <see cref="Type"/> is 'historicalTransaction', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransaction'.</exception>
    public KardFinancial.Users.CreateUploadPartResponseData AsHistoricalTransaction() =>
        IsHistoricalTransaction
            ? (KardFinancial.Users.CreateUploadPartResponseData)Value!
            : throw new global::System.Exception(
                "CreateUploadPartResponseDataUnion.Type is not 'historicalTransaction'"
            );

    public T Match<T>(
        Func<KardFinancial.Users.CreateUploadPartResponseData, T> onHistoricalTransaction,
    ...
    public void Visit(
        Action<KardFinancial.Users.CreateUploadPartResponseData> onHistoricalTransaction,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.Users.CreateUploadPartResponseData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransaction(
        out KardFinancial.Users.CreateUploadPartResponseData? value
    )
    {
        if (Type == "historicalTransaction")
        {
            value = (KardFinancial.Users.CreateUploadPartResponseData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransaction" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.Users.CreateUploadPartResponseData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.Users.CreateUploadPartResponseData"
                        ),
    ...
    {
        public HistoricalTransaction(KardFinancial.Users.CreateUploadPartResponseData value)
        {
            Value = value;
        }

        internal KardFinancial.Users.CreateUploadPartResponseData Value { get; set; }
