...
public record CreateUploadRequestDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.Users.StartHistoricalUploadNoData"/> if <see cref="Type"/> is 'historicalTransactionStart', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransactionStart'.</exception>
    public KardFinancial.Users.StartHistoricalUploadNoData AsHistoricalTransactionStart() =>
        IsHistoricalTransactionStart
            ? (KardFinancial.Users.StartHistoricalUploadNoData)Value!
            : throw new global::System.Exception(
                "CreateUploadRequestDataUnion.Type is not 'historicalTransactionStart'"
            );

    public T Match<T>(
        Func<KardFinancial.Users.StartHistoricalUploadNoData, T> onHistoricalTransactionStart,
    ...
    public void Visit(
        Action<KardFinancial.Users.StartHistoricalUploadNoData> onHistoricalTransactionStart,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.Users.StartHistoricalUploadNoData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransactionStart(
        out KardFinancial.Users.StartHistoricalUploadNoData? value
    )
    {
        if (Type == "historicalTransactionStart")
        {
            value = (KardFinancial.Users.StartHistoricalUploadNoData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransactionStart" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.Users.StartHistoricalUploadNoData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.Users.StartHistoricalUploadNoData"
                        ),
    ...
    {
        public HistoricalTransactionStart(KardFinancial.Users.StartHistoricalUploadNoData value)
        {
            Value = value;
        }

        internal KardFinancial.Users.StartHistoricalUploadNoData Value { get; set; }
