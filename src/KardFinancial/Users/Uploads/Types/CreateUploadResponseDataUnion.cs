...
public record CreateUploadResponseDataUnion
{
    ...
    /// <summary>
    /// Returns the value as a <see cref="KardFinancial.Users.CreateUploadResponseData"/> if <see cref="Type"/> is 'historicalTransactionStart', otherwise throws an exception.
    /// </summary>
    /// <exception cref="Exception">Thrown when <see cref="Type"/> is not 'historicalTransactionStart'.</exception>
    public KardFinancial.Users.CreateUploadResponseData AsHistoricalTransactionStart() =>
        IsHistoricalTransactionStart
            ? (KardFinancial.Users.CreateUploadResponseData)Value!
            : throw new global::System.Exception(
                "CreateUploadResponseDataUnion.Type is not 'historicalTransactionStart'"
            );

    public T Match<T>(
        Func<KardFinancial.Users.CreateUploadResponseData, T> onHistoricalTransactionStart,
    ...
    public void Visit(
        Action<KardFinancial.Users.CreateUploadResponseData> onHistoricalTransactionStart,
    ...
    /// <summary>
    /// Attempts to cast the value to a <see cref="KardFinancial.Users.CreateUploadResponseData"/> and returns true if successful.
    /// </summary>
    public bool TryAsHistoricalTransactionStart(
        out KardFinancial.Users.CreateUploadResponseData? value
    )
    {
        if (Type == "historicalTransactionStart")
        {
            value = (KardFinancial.Users.CreateUploadResponseData)Value!;
            return true;
        }
        value = null;
        return false;
        ...
        )
        {
            ...
            var value = discriminator switch
            {
                "historicalTransactionStart" =>
                    jsonWithoutDiscriminator.Deserialize<KardFinancial.Users.CreateUploadResponseData?>(
                        options
                    )
                        ?? throw new JsonException(
                            "Failed to deserialize KardFinancial.Users.CreateUploadResponseData"
                        ),
    ...
    {
        public HistoricalTransactionStart(KardFinancial.Users.CreateUploadResponseData value)
        {
            Value = value;
        }

        internal KardFinancial.Users.CreateUploadResponseData Value { get; set; }
